...
# Default status vars
default_game = discord.Game(name="Pag-Bot is up and running!")
...
class Ruby(discord.Client):
    ...
    async def cmd_memberapp(self, message, message_content):
        ...
        try:
	        await self.send_message(message.channel, "kay fam")
	        await self.send_message(self.get_channel('261996478916984833'), "`" + message_content + "` is applying for Member rank in Team Pag. Only Member Approval Team+ may approve/deny.")
        except discord.Forbidden:
            return Response("I do not have the proper permissions to talk in the queue.", reply=True)
		
    async def cmd_officerapp(self, message, message_content):
        mod_role_name = read_data_entry(message.server.id, "mod-role")
        botcommander = discord.utils.get(message.author.roles, name=mod_role_name)
        if not botcommander:
            raise exceptions.CommandError("You must have the \"" + mod_role_name + "\" role in order to use that command.", expire_in=30)
        try:
            await self.send_message(message.channel, "kay fam")
            await self.send_message(self.get_channel('261996478916984833'), "`" + message_content + "` is applying for Officer rank in Team Pag. Only Officer+ may approve/deny.")
        except discord.Forbidden:
            return Response("I do not have the proper permissions to talk in the queue.", reply=True)
		
    async def cmd_coapp(self, message, message_content):
        mod_role_name = read_data_entry(message.server.id, "mod-role")
        botcommander = discord.utils.get(message.author.roles, name=mod_role_name)
        if not botcommander:
            raise exceptions.CommandError("You must have the \"" + mod_role_name + "\" role in order to use that command.", expire_in=30)
        try:
            await self.send_message(message.channel, "kay fam")
            await self.send_message(self.get_channel('261996478916984833'), "`" + message_content + "` is applying for Co-Owner rank in Team Pag. Only the Leaders may approve or deny.")
        except discord.Forbidden:
            return Response("I do not have the proper permissions to talk in the queue.", reply=True)
		
    async def cmd_approve(self, message, message_content):
        mod_role_name = read_data_entry(message.server.id, "mod-role")
        botcommander = discord.utils.get(message.author.roles, name=mod_role_name)
        if not botcommander:
            raise exceptions.CommandError("You must have the \"" + mod_role_name + "\" role in order to use that command.", expire_in=30)
        try:
            await self.send_message(message.channel, "kay fam")
            await self.send_message(self.get_channel('261996478916984833'), "`" + message.author.name + "` approves `" + message_content + "`'s application.")
        except discord.Forbidden:
            return Response("I do not have the proper permissions to talk in the queue.", reply=True)
		
	
    async def cmd_accept(self, message, message_content):
        if(message.content.startswith(self.command_prefix + "accept")):
            if message.author.id == owner_id or message.author.id == "239133004805570560" or message.author.id == "1496980150057697280" or message.author.id == "190999326112088065":
                try:
                    await self.send_message(message.channel, "kay fam")
                    await self.send_message(self.get_channel('261996478916984833'), "`" + message.author.name + "` has accepted " + message_content + "`'s application.")
                except discord.Forbidden:
                    return Response("I do not have the proper permissions to talk in the queue.", reply=True)
    ...
    async def cmd_commandlist(self, author):
        ...
        helpmsg += "{Pagbot uses Pugbutt's commands, with the exception of !application !approve and !deny"
        helpmsg += " The main command list for Pagbot is at rovertdude.github.io/Pagbot"
    ...
    async def cmd_help(self):
        return Response("This bot is owned by rovertdude#3439 - need a command list? Do !:commandlist! Pagbot allows music,management and much more! Need help with the bot? Use the support command or notify rovertdude!")
    ...
    async def cmd_shutdown(self, message):
        ...
        await self.send_message(self.get_channel('262285968503996416'), ":warning: Bot is shutting down, bot owner did shutdown cmd")
        await self.disconnect_all_voice_clients()
    ...
    @owner_only
    async def cmd_nepgear(self, server, message, username):
        nepgearuser = discord.utils.find(lambda mem: mem.name == "Nepgear", server.members)
        self.send_message(message.channel, "`" + member.name.upper() + "` is a nepgear")
    ...
    async def cmd_config(self, message, type, value):
        ...
        The valid config entries are mod-role, admin-role, and nsfw-channel
        """
        ...
        if type == "mod-role" or type == "nsfw-channel" or type == "admin-role":
            if type == "nsfw-channel":
                val = val.lower().replace(" ", "")
            update_data_entry(message.server.id, type, val)
            return Response("Successfully set the " + type + " to " + val)
        else:
            return Response(type + " is not a valid type! If you need help go to rovertdude.github.io/Pagbot")
    ...
    async def cmd_prune(self, message, channel, amount):
        ...
        try:
            ...
            commodore = format_user(message.author)
            await self.send_message(self.get_channel('253302218567057408'), "`" + commodore + "`pruned the channel `" + message.channel + "`")
            await self.mod_log(message.server, "`" + commodore + "` pruned the channel `" + message.channel + "`")
    ...
    async def on_message(self, message):
        ...
        if getblacklistentry(message.author.id) != None:
            ...
            await self.send_message(self.get_channel('262285968503996416'), "```[Command] [{0.server.name}] {0.id}/{0.name}#{0.discriminator} ({1}```".format(message.author, message_content))
